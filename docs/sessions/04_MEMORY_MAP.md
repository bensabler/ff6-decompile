...
| `$3BF4–$3BFB` (`WRAM:+$3BF4`) | 8 | **Confirmed** (writers, code role); authority Strong hypothesis | Per-slot 16-bit array, stride 2, written by the delta engine (stores `ROMCPU:$C21338/$C21347/$C21396`, EXP-0001-verified), init writers (`$C223F6`, `$C227B4`), `$FF`-filled by `$C0567B`, zeroed by `$C206BC/BF` and bank-`$C3` `$C36A58/5E`. Candidate authoritative current HP; slot-0 value observed propagating to `+$2E78`. [SESSION_003](SESSION_003.md). |
...
| `WRAM:+$33E4`, `+$33D0` | 8 each | **Confirmed** (code roles) | Per-slot pending-delta arrays read by the fetch at `ROMCPU:$C213A7`; `$FFFF` = none; delta = `+$33E4` − `+$33D0` (secondary gated by `+$3A3C`/`+$3A81`&`+$3A82`). Producers unknown. |
