...
| `WRAM:+$3AB4` | `$14` (10 × 2) | **Confirmed** (EXP-0043) | **ATB gauge array.** Ten entries, **stride 2**, 16-bit; party 0–3, enemies 4–9. Advanced by `$3AC8,X >> 1` at `ROMCPU:$C21195–$C2119D`. Observed advancing by exactly `$4E` = `$9C >> 1` and wrapping (`$00B6` → `$0004`). Note the stride is **2**, not the `$14` of the HP/stat family. |
| `WRAM:+$3AC8` | `$14` (10 × 2) | **Confirmed** (EXP-0043) | **Per-slot ATB increment.** Built near `ROMCPU:$C209E0` from the slot's Speed at `+$3B19,X`; the `+$3A90` (Battle Speed) multiply at `$C209FA` applies **only when X ≥ 8**, i.e. enemy slots. Measured: party 318/330/336 identical at Bat.Speed 3 and 6; enemies 240 vs 156. Party values fit `(Speed + 20) × 6`. |
| `WRAM:+$3AA0` | `$14` (10 × 2) | **Confirmed** (code role, EXP-0043) | Per-slot scheduler flag word, stride 2. Tested at `ROMCPU:$C2114B` before a slot is advanced; written on the threshold path at `$C211B2` (`ORA #$20`). Individual bit semantics **Unknown** — bit 6 was observed set transiently at a gauge advance. |
| `WRAM:+$3A3E`–`+$3A3F` | 2 | **Confirmed** (EXP-0043) | 16-bit **battle tick counter**, `INC`'d once per non-gated frame at `ROMCPU:$C21136`. Observed ≈0.5 increments per emulator frame in one window; still increments on the defeat screen. |
| `WRAM:+$3218` | `$14` (10 × 2) | **Confirmed** (code role, EXP-0043) | Second per-slot accumulator: `$3218,X += $3AC8,X` at `ROMCPU:$C211BB`, gated on `$3219,X`. Relationship to `+$3AB4` unresolved — charge/delay timer candidate. |
| `WRAM:+$3B19,X` (stride 2) | — | Strong hypothesis (EXP-0043) | Per-slot **Speed** stat feeding the ATB increment (`LDA $3B19,X / ADC #$14` at `$C209F0`). Read 33/35/36 for the opening party and 30 for each Were-Rat. Distinct from the `$14`-stride `+$3B18` stat block. |
