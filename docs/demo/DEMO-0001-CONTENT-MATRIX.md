...
- **Updated:** 2026-08-02 (Unit 13 — EXP-0050 re-scoped the compression dependency)
...
|---|---|---|---|---|---|
| CD-CMP-01 | FF6 compression format | X1 | **EXP-0050**: 47-52 % of a field scene's VRAM is verbatim ROM, including all three contiguous BG tile blocks; battle 38 %; Mode 7 opening 0 % | `Unknown` | **No longer the keystone this file first called it.** It does not gate CD-MAP-02 on this route. What it gates is unknown until the unmatched remainder is characterised — and "unmatched" is not "compressed" |
...
| CD-TXT-06 | Fixed-width HUD font tiles | T1 | GFX-0001, EXP-0023, ROM-0016 `ROMFILE:0x047FB0-0x048FBF`; **EXP-0050** adds the destination: it loads to `VRAM:$AFF0` | `Validated` | the copying routine is still unlocated |
...
| ID | Dependency | Readiness | Anchor | State | Blocker / next action |
|---|---|---|---|---|---|
| CD-SPR-01 | Field sprite tiles | F6 | CEN-GFX-0002 observed | `Unknown` | not yet swept with `ff6lab state origin`; do it alongside CD-SPR-02 |
| CD-SPR-02 | OAM / metasprite layout | F6 | CEN-GFX-0008: every savestate carries `ppu.oamRam`, and `ff6lab state oam` now reaches it | `Runtime Captured` | **still never examined.** The cheapest unstarted graphics unit |
| CD-SPR-03 | Walking frames and animation timing | F4, F6 | none | `Unknown` | CD-SPR-01 |
| CD-SPR-04 | Party battle sprites | B16 | **EXP-0050**: same banks `$D8`/`$E9` short runs as CD-SPR-05 | `Located` | separate enemy from party runs |
| CD-SPR-05 | Enemy graphics | B15 | CEN-MONSTER-0003; **EXP-0050**: battle sprite regions `VRAM:$6180-$7829` map to short verbatim runs in banks `$D8`/`$E9` | `Located` | separate enemy from party runs |
| CD-SPR-06 | Whelk head and shell graphics | B15 | head/shell visually classifiable at 4× (EXP-0040) | `Unknown` | CD-BAT-13; no Whelk battle savestate has been swept |
...
| CD-BAT-04 | Battle backgrounds | B14 | CEN-GFX-0003 observed; **EXP-0050**: the battle scene's BG chr region `VRAM:$4000-$47FF` is **not** verbatim in ROM, while `$4800-$4FFF` maps to `ROMFILE:0x225700` | `Unknown` | the strongest remaining compression candidate on this route |
...
| B02 Opening cinematic | auto-runs without input to first input-waiting dialogue (EXP-0031) | CD-MAP-01…04, CD-MAP-07, CD-EVT-01/02/03, CD-TXT-01/03, CD-SPR-01/03, CD-AUD-01/04 | CD-MAP-01, CD-EVT-01, CD-TXT-01 |
| B03 Snowfield march | no capture | CD-MAP-01…04, CD-EVT-05, CD-SPR-01/03, CD-AUD-01 | CD-MAP-01, CD-EVT-05 |
...
| B06 Entry into Narshe | milestone `02-narshe-entry`, deterministic (EXP-0032) | CD-MAP-01…06, CD-EVT-01 | CD-MAP-01, CD-MAP-06 |
...
| B11 Mine traversal | corridor `(26,1C)`→`(26,0B)`→`(28,0B)`→`(28,09)`→`(2A,09)` (EXP-0038) | CD-MAP-01…05, CD-EVT-07 | CD-MAP-01, CD-MAP-05 |
...
| ~~CD-CMP-01 compression~~ | **withdrawn** — see below | X1 |

### The first version of this table was wrong, and EXP-0050 is why

When this file was created it ranked compression first, blocking 8 of 19
beats, on the strength of readiness X1's prose. That figure was inherited, not
measured: X1 said compression gated maps, field sprites, battle backgrounds,
enemy graphics and party sprites, and this table counted the beats those rows
touch.

**EXP-0050 tested the premise and it did not hold.** A verbatim search against
preserved VRAM finds the BG tile data for milestones 02, 04 and 05 present in
the ROM **uncompressed**, in three contiguous spans totalling 20 KB, shared
between the Narshe exterior and the mines interior. No decompressor is
involved in placing them.

Compression is therefore withdrawn from this table rather than re-ranked,
because its true count is **unknown**: 48-62 % of each VRAM image is unmatched,
and a verbatim search is defeated by bit-plane reordering, runtime composition
and tilemaps built in WRAM as readily as by compression. Ranking it on the
unmatched fraction would repeat the original mistake in the other direction.

**Map headers now lead** — and unlike before, F1 has concrete anchors:
`ROMFILE:0x208460` and `0x223000` are the blocks a header must be selecting.
