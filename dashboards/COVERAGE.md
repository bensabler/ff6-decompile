...
- **CEN-EVENT-0001** (Event scripting engine (opening scripted sequence)): CORR-0001 settled $C09B5C's mechanism and located candidate dispatcher/table. Next: exec-watch the dispatcher's JMP ($002A) at ROMCPU:$C09B59 alongside $C09B5C, capturing DP $2A/$2B and $EA at dispatch, to name the immediate predecessor per entry and turn 'A is a command length' into a measured opcode-to-length table (decodes ROMCPU:$C098C4 as a by-product).
